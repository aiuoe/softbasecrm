        ...
        public async Task<PagedResultDto<CustomerWipViewDto>> GetPagedCustomerWip(GetAllCustomerWipInput input)
        {
            try
            {
                var filteredCustomerWip = from wipList in _dbContext.Set<WIPList>()
                        ...
                        join customers in _dbContext.Set<Customer>() on wo.ShipTo equals customers.Number into c from customer in c.DefaultIfEmpty()
                        join equipments in _dbContext.Set<Equipment>() on wipList.SerialNo equals equipments.SerialNo into e from equipment in e.DefaultIfEmpty()
